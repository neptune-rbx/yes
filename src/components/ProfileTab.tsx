...
export function ProfileTab({ tracker, year, setYear, showAuthModal, setShowAuthModal }: { tracker: ReturnType<typeof useTracker>; year: number; setYear: (y: number) => void; showAuthModal: boolean; setShowAuthModal: (v: boolean) => void }) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-10 animate-in pb-12">
      {session ? (
        ...
        <>
          {/* Auth Only Section - Show when NOT logged in */}
          <div className="bg-zinc-900/20 border border-zinc-900 rounded-2xl p-8">
            <h3 className="text-lg font-bold uppercase tracking-widest text-zinc-300 mb-6">Sign In to Get Started</h3>
            
            <div className="space-y-6">
              <p className="text-sm text-zinc-400">Create an account or sign in to sync your ride counts across devices.</p>
